...
script_dir <- local({
  ...
  script_path <- if (length(file_arg) > 0) sub("^--file=", "", file_arg[1]) else "fitbit_stim_cued_interaction.R"
  normalizePath(dirname(script_path), winslash = "/", mustWork = FALSE)
})
source(file.path(script_dir, "..", "glmm_common.R"))

ctx <- init_glmm_script("fitbit_stim_cued_interaction.R")
...
df <- df[!is.na(df$fitbit_overall_motion), , drop = FALSE]

continuous_cols <- c("fitbit_stim_motion")
...
log_cat("Subgroup: Fitbit-measured nights (non-missing fitbit_stim_motion)\n")
log_cat(
  "Note: interaction version keeps cued and non-cued nights ",
  "(unlike fitbit_stim_cued.R, which is cued nights only).\n",
  sep = ""
)
log_cat("Rows before complete-case filtering:", nrow(df), "\n")
log_cat("Rows used in fitbit stim interaction:", nrow(model_df), "\n")
...
run_lucid_glmm(
  formula = formula,
  data = model_df,
  model_label = "model3_fitbit_stim_motion_cueing_interaction",
  script_name = ctx$script_name,
  results_dir = ctx$results_dir,
  timestamp = ctx$timestamp,
  summary_title = "Model 3 Fitbit Stim Cueing Interaction Summary (glmmTMB binomial)"
)
