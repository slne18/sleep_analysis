...
script_dir <- local({
  ...
  script_path <- if (length(file_arg) > 0) sub("^--file=", "", file_arg[1]) else "phone_overall.R"
  normalizePath(dirname(script_path), winslash = "/", mustWork = FALSE)
})
source(file.path(script_dir, "..", "glmm_common.R"))

ctx <- init_glmm_script("phone_overall.R")
...
run_lucid_glmm(
  formula = formula,
  data = model_df,
  model_label = "model3_phone_overall_motion",
  script_name = ctx$script_name,
  results_dir = ctx$results_dir,
  timestamp = ctx$timestamp,
  summary_title = "Model 3 Phone Overall Summary (glmmTMB binomial)"
)
