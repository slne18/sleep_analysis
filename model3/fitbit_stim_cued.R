...
script_dir <- local({
  ...
  script_path <- if (length(file_arg) > 0) sub("^--file=", "", file_arg[1]) else "fitbit_stim_cued.R"
  normalizePath(dirname(script_path), winslash = "/", mustWork = FALSE)
})
source(file.path(script_dir, "..", "glmm_common.R"))

ctx <- init_glmm_script("fitbit_stim_cued.R")
...
log_cat("Subgroup: Fitbit-measured cued nights (non-missing fitbit_stim_motion)\n")
log_cat("Rows before complete-case filtering:", nrow(df), "\n")
print_predictor_preprocessing(continuous_cols)

formula <- as.formula(
  paste("lucid ~", paste(predictor_cols, collapse = " + "), "+ (1 | pid)")
)

run_lucid_glmm(
  formula = formula,
  data = model_df,
  model_label = "model3_fitbit_stim_motion_cued",
  script_name = ctx$script_name,
  results_dir = ctx$results_dir,
  timestamp = ctx$timestamp,
  summary_title = "Model 3 Fitbit Stim Cued Summary (glmmTMB binomial)"
)
