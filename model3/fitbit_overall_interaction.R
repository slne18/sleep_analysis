...
script_dir <- local({
  ...
  script_path <- if (length(file_arg) > 0) sub("^--file=", "", file_arg[1]) else "fitbit_overall_interaction.R"
  normalizePath(dirname(script_path), winslash = "/", mustWork = FALSE)
})
source(file.path(script_dir, "..", "glmm_common.R"))

ctx <- init_glmm_script("fitbit_overall_interaction.R")
...
continuous_cols <- c("fitbit_overall_motion")
...
log_cat("Subgroup: Fitbit-measured nights (non-missing fitbit_overall_motion)\n")
log_cat("Rows before complete-case filtering:", nrow(df), "\n")
log_cat("Rows used in fitbit overall interaction:", nrow(model_df), "\n")
...
run_lucid_glmm(
  formula = formula,
  data = model_df,
  model_label = "model3_fitbit_overall_motion_cueing_interaction",
  script_name = ctx$script_name,
  results_dir = ctx$results_dir,
  timestamp = ctx$timestamp,
  summary_title = "Model 3 Fitbit Overall Cueing Interaction Summary (glmmTMB binomial)"
)
