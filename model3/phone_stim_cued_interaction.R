...
script_dir <- local({
  ...
  script_path <- if (length(file_arg) > 0) sub("^--file=", "", file_arg[1]) else "phone_stim_cued_interaction.R"
  normalizePath(dirname(script_path), winslash = "/", mustWork = FALSE)
})
source(file.path(script_dir, "..", "glmm_common.R"))

ctx <- init_glmm_script("phone_stim_cued_interaction.R")
...
log_cat(
  ...
  "(unlike phone_stim_cued.R, which is cued nights only).\n",
  sep = ""
)
log_cat("Rows before complete-case filtering:", nrow(df), "\n")
log_cat("Rows used in phone stim interaction:", nrow(model_df), "\n")
...
run_lucid_glmm(
  formula = formula,
  data = model_df,
  model_label = "model3_phone_stimulation_cueing_interaction",
  script_name = ctx$script_name,
  results_dir = ctx$results_dir,
  timestamp = ctx$timestamp,
  summary_title = "Model 3 Phone Stim Cueing Interaction Summary (glmmTMB binomial)"
)
