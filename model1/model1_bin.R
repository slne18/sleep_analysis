...
script_path <- if (length(file_arg) > 0) sub("^--file=", "", file_arg[1]) else "model1_bin.R"
...
cat("Rows used in Model 2:", nrow(model_df), "\n")

continuous_predictors <- paste0(scale_cols, predictor_suffix)
formula_str <- paste0(
  "lucid ~ had_arousal + cued_night + sleep_quality + ",
...
cat("\n===== Model 2 Summary (glmmTMB binomial) =====\n")
