...
script_path <- if (length(file_arg) > 0) sub("^--file=", "", file_arg[1]) else "submodel5.R"
...
allowed_fitbit_phone_keys <- motion_raw %>%
  mutate(
  ...
  filter(has_fitbit_phone) %>%
  select(pid_char, night_number)
fitbit_phone_pid_count <- dplyr::n_distinct(allowed_fitbit_phone_keys$pid_char)
...
model_df <- base_df %>%
  ...
  semi_join(
    allowed_fitbit_phone_keys,
    by = c("pid_char", "inferred_night_number" = "night_number")
  ) %>%
  filter(
    lucid %in% c(0, 1),
    complete.cases(
      lucid, totalCues, fitbit_stim_motion, lucid_attempts_past_week, lucid_dreams_past_week,
      age, wake_up_duration_min, sleep_quality_num, wakeThresh, pid
    )
  )
cat("Fitbit+Phone pid/night keys kept:", nrow(allowed_fitbit_phone_keys), "\n")
cat("Fitbit+Phone unique PID count:", fitbit_phone_pid_count, "\n")
...
cat("Rows used in submodel5 (totalCues > 0):", nrow(cued_df), "\n")
...
cat("\n===== Submodel5 Summary (log version, glmmTMB binomial) =====\n")
print(summary(submodel5))

cat("\n===== Odds Ratios =====\n")
print(exp(fixef(submodel5)$cond))
...
cat("\n===== DHARMa Diagnostics =====\n")
simulationOutput <- simulateResiduals(submodel5)
