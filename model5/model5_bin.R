...
script_path <- if (length(file_arg) > 0) sub("^--file=", "", file_arg[1]) else "model5_bin.R"
...
cat("Rows used in Model 5:", nrow(model_df), "\n")
...
cat("\n===== Model 5 Summary (glmmTMB binomial) =====\n")
...
plot_file <- file.path(results_dir, "Rplots.pdf")
pdf(plot_file)
