...
script_path <- if (length(file_arg) > 0) sub("^--file=", "", file_arg[1]) else "logistic_cue_delta_var.R"
...
fit <- glm(lucid ~ cue_delta_var, data = model_df, family = binomial(link = "logit"))
