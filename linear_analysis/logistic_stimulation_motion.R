...
script_path <- if (length(file_arg) > 0) sub("^--file=", "", file_arg[1]) else "logistic_stimulation_motion.R"
...
fit <- glm(lucid ~ log(stimulation_motion + 1), data = model_df, family = binomial(link = "logit"))
