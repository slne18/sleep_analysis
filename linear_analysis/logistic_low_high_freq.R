...
script_path <- if (length(file_arg) > 0) sub("^--file=", "", file_arg[1]) else "logistic_low_high_freq.R"
...
fit <- glm(lucid ~ low_freq + high_freq, data = model_df, family = binomial(link = "logit"))
