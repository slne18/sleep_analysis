...
script_path <- if (length(file_arg) > 0) sub("^--file=", "", file_arg[1]) else "logistic_fitbit_stim_motion.R"
...
fit <- glm(lucid ~ fitbit_stim_motion, data = model_df, family = binomial(link = "logit"))
